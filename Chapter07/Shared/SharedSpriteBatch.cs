...
namespace GameStateManager
{
    ...
    public class SharedSpriteBatch
    {
        ...
        private void Begin(SpriteBlendMode spriteBlendMode)
        {
            ...
            if (_mode != spriteBlendMode || _sortMode != SpriteSortMode.Deferred)
            {
        ...
        private void Begin(SpriteBlendMode spriteBlendMode, SpriteSortMode spriteSortMode)
        {
            // Started, but changing
            if (_mode != spriteBlendMode || _sortMode != spriteSortMode)
            {
                End();
            }
            ...
            if (!_hasBegun)
            {
                ...
                _spriteBatch.Begin(_mode, _sortMode, SaveStateMode.None);
                _hasBegun = true;
            }
        }
        ...
        public void Draw(SpriteSortMode sortMode, Texture2D texture, Vector2 position, Rectangle? srcRect,
            Color color, float rotation, Vector2 origin, float scale, SpriteEffects effects, float layer)
        {
            Begin(_mode, sortMode);
            _spriteBatch.Draw(texture, position, srcRect, color, rotation, origin, scale, effects, layer);
        }
